...
- ASP.NET Core sets separate API and browser CSP policies, permissions, anti-framing, referrer, and content-type headers, and makes `/assets/config.js` non-cacheable.
...
- Bicep contains no broad Azure-services SQL firewall rule. The deployment workflow removes a legacy rule if present, reconciles App Service outbound-IP rules, and always cleans up its temporary runner rule. SCM/FTP basic publishing authentication is disabled.
...
- There is no external secret-provider wiring, dedicated secret-scanning job, dependency-update automation, or Docker image scanning. CI does run npm and NuGet vulnerability checks. GitHub Actions uses OIDC, but federation does not replace deliberate production secret configuration.
...
5. Use deployment OIDC with Contributor scoped only to the portfolio resource group and a protected GitHub `production` environment; do not create a long-lived Azure client secret where federation is available.
