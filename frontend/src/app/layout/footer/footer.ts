...
@Component({
  ...
  styles: [`footer{display:grid;grid-template-columns:1fr auto;gap:1.5rem;padding:2rem clamp(1rem,3vw,2rem);border-top:1px solid var(--line);color:var(--slate)}strong{color:var(--paper)}p{margin:.35rem 0 0}.footer-links{display:flex;gap:1rem;font:600 .7rem var(--mono)}.footer-links a{color:var(--signal);text-underline-offset:.25rem}footer>p{grid-column:1/-1;font-size:.75rem}@media(max-width:600px){footer{grid-template-columns:1fr}.footer-links{flex-wrap:wrap}}`],
})
export class Footer {
